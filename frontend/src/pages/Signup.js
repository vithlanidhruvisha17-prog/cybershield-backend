    ...
    const handleSignup = async (e) => {
        ...
        try {
            const res = await axios.post(`${API_URL}/api/signup`, {
                fullname: formData.fullname, email: formData.email, username: formData.username, password: formData.password
            });
            if (res.data.success) {
                        ...
                        <form className="space-y-4" onSubmit={handleSignup}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-1">
                                    ...
                                    <input type="email" id="signup-email" onChange={handleChange} placeholder="Email" className="w-full p-3 bg-[#0e0e0e] border border-white/5 rounded-xl outline-none text-sm" required />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[#FFD700] text-[10px] font-bold uppercase tracking-widest ml-1">Username</label>
                                    <input type="text" id="signup-username" onChange={handleChange} placeholder="Username" className="w-full p-3 bg-[#0e0e0e] border border-white/5 rounded-xl outline-none text-sm" required />
                                </div>
                            </div>
                            <div className="space-y-1">
                                <label className="text-[#FFD700] text-[10px] font-bold uppercase tracking-widest ml-1">Password ({strength})</label>
                                <input type="password" id="signup-password" onChange={handleChange} placeholder="Strong Password" className="w-full p-3 bg-[#0e0e0e] border border-white/5 rounded-xl outline-none text-sm" required />
                            </div>
                            <div className="space-y-1">
                                <label className="text-[#FFD700] text-[10px] font-bold uppercase tracking-widest ml-1">Confirm Password</label>
                                <input type="password" id="signup-confirmPassword" onChange={handleChange} placeholder="Confirm Password" className="w-full p-3 bg-[#0e0e0e] border border-white/5 rounded-xl outline-none text-sm" required />
                            ...
                            <p className="text-center text-xs text-gray-500 mt-4">Already have an account? <Link to="/login" className="text-[#FFD700] font-black hover:underline ml-1">Login</Link></p>
                        </form>
                    </div>
                </div>
            </main>
